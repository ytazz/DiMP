...
public:
	...
	DiMP2::DrawCanvasGL     drawCanvasGL;
	DiMP2::DrawCanvasSVG    drawCanvasSVG;
...
public: /** 派生クラスが実装する関数 **/
	...
	virtual void OnStep(){
		...
		for(int i = 0; i < nvar; i++)
			...
			//deltaNorm = std::max(deltaNorm, graph->solver.vars[i]->d.square());
...
public: /** FWAppの実装 **/
	...
	virtual void Init(int argc, char* argv[]){
		...
		windowDesc.width  = 1400;
		windowDesc.height = 1000;
	...
	virtual void Keyboard(int key, int x, int y) {
		...
		/* デバッグ用
		if(key == 'g')
			graph->solver.hoge = !graph->solver.hoge;
		 */
		/* 計算時間計測
		if(key == 't'){
			int t0 = timeGetTime();
			for(int i = 0; i < 1000; i++)
				graph->Step();
			int t1 = timeGetTime();
			DSTR << "elapsed time[ms]: " << t1 - t0 << endl;
		}
		*/
