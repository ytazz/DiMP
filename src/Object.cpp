...
void ObjectKey::Draw(DrawCanvas* canvas, DrawConfig* conf){
	Object* obj = (Object*)node;
	Vec3f p0, p1;
	
	// position
	if(conf->Set(canvas, DrawItem::ObjectPos, obj)){
		p0 = pos_t->val;
		canvas->Point(p0);
	}
	// velocity
	if(conf->Set(canvas, DrawItem::ObjectVel, obj)){
		float s = conf->Scale(DrawItem::ObjectVel, obj);
		p1 = p0 + s * vel_t->val;
		canvas->Line(p0, p1);
	}
	// ang-velocity
	if(conf->Set(canvas, DrawItem::ObjectAngvel, obj)){
		float s = conf->Scale(DrawItem::ObjectAngvel, obj);
		p1 = p0 + s * vel_r->val;
		canvas->Line(p0, p1);
	}
}
...
void Object::Draw(DrawCanvas* canvas, DrawConfig* conf){
	if(conf->Set(canvas, DrawItem::ObjectTrajectory, this)){
...
void Object::DrawTrajectory(DrawCanvas* canvas, uint ndiv){
	if(traj.empty())
		return;
...
void Object::DrawSnapshot(real_t time, DrawCanvas* canvas, DrawConfig* conf){
	pose_t pose;
...
void Object::DrawSnapshot(const pose_t& pose, DrawCanvas* canvas, DrawConfig* conf){
	Affinef aff;
