...
//初期条件を与えるクラス
void BipedLIP::beginning(){


	BipedLIPKey* key = GetSegment(0).first;		//GetSegment(任意の時刻).first

		key->pos_t[0]->val = 0.0;	//CoM
		key->pos_t[1]->val = 0.0;	//

		key->pos_cop[0]->val = param.rfoot_place.x;	//CoP
		key->pos_cop[1]->val = param.rfoot_place.y;
//		cout << key->pos_cop[0]->val << "，" << key->pos_cop[1]->val <<  endl;

		key->vel_t[0]->val = 0.0;
		key->vel_t[1]->val = 0.5;
//		cout << key->vel_t[0]->val << "，" << key->vel_t[1]->val <<  endl;
...
float prepos_right_x = 0.0, prepos_left_x = 0.0, temp_right_x = 0.0, temp_left_x = 0.0;	//一つ前の計算での最終足先位置_x
float prepos_right_y = 0.0, prepos_left_y = 0.0, temp_right_y = 0.0, temp_left_y = 0.0;	//一つ前の計算での最終足先位置_y
float dif_right_x, dif_left_x,dif_right_y, dif_left_y;	//一つ前の計算との差
...
void BipedLIP::Write_csv(Vec3f p_r,Vec3f p_l,real_t t,real_t dt){
	...
	dif_right_x = abs(prepos_right_x - temp_right_x);	//一つ前の計算での最終足先位置差
	dif_left_x = abs(prepos_left_x - temp_left_x);
	dif_right_y = abs(prepos_right_y - temp_right_y);
	dif_left_y = abs(prepos_left_y - temp_left_y);

			
	if(t == 0 && dif_left_x == 0 && dif_right_x == 0 && dif_left_y == 0 && dif_right_y == 0){	//差がない(小さい)とき
		cnt++;
	}else if(t == 0){	//一つ前の最終足先位置を格納
		prepos_right_x = temp_right_x;
		prepos_left_x = temp_left_x;
		prepos_right_y = temp_right_y;
		prepos_left_y = temp_left_y;
	}
	...
	*/
	...
	if (counter % output_count == 0){ 
		temp_right_x = pos_right.x;
		temp_left_x = pos_left.x;
		temp_right_y = pos_right.y;
		temp_left_y = pos_left.y;
	}
