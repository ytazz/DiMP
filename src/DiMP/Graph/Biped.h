...
namespace DiMP {;
	...
	*/
	...
	public:
		...
		int    Phase          (real_t t);
		virtual void   ComState       (real_t t, vec3_t& pos, vec3_t& vel, vec3_t& acc);
		...
		virtual void   FootPose       (real_t t, int side, pose_t& pose, vec3_t& vel, vec3_t& angvel, vec3_t& acc, vec3_t& angacc, int& contact);
		void   FootCopState   (real_t t, int side, vec3_t& pos, vec3_t& vel, real_t& weight);
		virtual real_t TimeToLiftoff  (real_t t, int side);
		virtual real_t TimeToLanding  (real_t t, int side);
