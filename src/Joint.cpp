...
void JointKey::Draw(DrawCanvas* canvas, DrawConfig* conf){
	Joint* jnt = (Joint*)node;
	Vec3f p0, p1;
	...
	// joint force
	if(conf->Set(canvas, DrawItem::JointForce, jnt)){
		real_t s = conf->Scale(DrawItem::JointForce, jnt);
		p0 = pf;
		p1 = p0 + s * force_t->val;
		canvas->Line(p0, p1);
	}
	// joint moment
	if(conf->Set(canvas, DrawItem::JointMoment, jnt)){
		real_t s = conf->Scale(DrawItem::JointMoment, jnt);
		p0 = pf;
		p1 = p0 + s * force_r->val;
		canvas->Line(p0, p1);
	}
	
	if(conf->Set(canvas, DrawItem::Connector, jnt)){
...
void Joint::DrawSnapshot(real_t time, DrawCanvas* canvas, DrawConfig* conf){
...
void Genericjoint::OnDraw(real_t* pos, DrawCanvas* canvas){
	if(callback)
