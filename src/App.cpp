...
App::App(){
	graph         = new Graph();
	drawConf      = 0;
	drawCanvasGL  = new DrawCanvasGL ();
	drawCanvasSVG = new DrawCanvasSVG();
	showHelp      = true;
	appName	      = "untitled";
...
void App::SaveSVG(){
	...
	drawCanvasSVG->SetViewportSize(sz.x, sz.y);

	Affinef affProj, affView;
	render->GetProjectionMatrix(affProj);
	affView = win->GetTrackball()->GetAffine();

	drawCanvasSVG->SetProjMatrix(affProj);
	drawCanvasSVG->SetViewMatrix(affView);
	drawCanvasSVG->svg.SetScale (1000.0f, -1000.0f);
	drawCanvasSVG->svg.SetOffset(0.0f, 1000.0f);
	drawCanvasSVG->svg.SetAspect(aspect);
	drawCanvasSVG->svg.SaveStart("hoge.svg", 1000, 1000);
	if(zAxisUp){
		drawCanvasSVG->Push  ();
		drawCanvasSVG->Rotate((float)Rad(-90.0), Vec3f(1.0f, 0.0f, 0.0f));
	}
	
	graph->Draw(drawCanvasSVG, drawConf);
	
	if(zAxisUp){
		drawCanvasSVG->Pop();
	}	
	drawCanvasSVG->svg.SaveEnd();
}
...
void App::OnDraw(GRRenderIf* render){
	...
	graph->Draw(drawCanvasGL, drawConf);

	if(GetAction(MENU_ALWAYS, ID_PLAY)->GetBool()){
		graph->DrawSnapshot(playTime, drawCanvasGL);
	}
	if(zAxisUp)
...
void App::OnAction(int menu, int id){
	...
	if(menu == MENU_ALWAYS){
		...
		if(id == ID_LOG){
			graph->solver->EnableLogging(DiMP2::Logging::MajorLoop, act->GetBool());
		}
		if(id == ID_SVG){
			SaveSVG();
		}
...
void App::Init(int argc, char* argv[]){
	...
	tb->SetDistance(3.0f);
	tb->SetLatitudeRange(Rad(-90.0), Rad(90.0));
