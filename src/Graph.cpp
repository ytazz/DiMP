...
void Graph::Draw(DrawCanvas* canvas, DrawConfig* conf){
	if(!conf)
		conf = drawConf;

	if(conf->Set(canvas, DrawItem::GlobalAxis, 0)){
		float l = conf->Scale(DrawItem::GlobalAxis, 0);
		canvas->Line(Vec3f(), Vec3f(l   , 0.0f, 0.0f));
		canvas->Line(Vec3f(), Vec3f(0.0f, l   , 0.0f));
		canvas->Line(Vec3f(), Vec3f(0.0f, 0.0f, l   ));
	}

	nodes.Draw(canvas, conf);
	
	for(int i = 0; i < (int)ticks.size(); i++)
		DrawSnapshot(ticks[i]->time, canvas, conf);
}

void Graph::DrawSnapshot(real_t time, DrawCanvas* canvas, DrawConfig* conf){
	if(!conf)
		conf = drawConf;

	trajNodes.DrawSnapshot(time, canvas, conf);
}
